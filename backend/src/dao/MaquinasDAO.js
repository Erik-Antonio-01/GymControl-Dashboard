...
export default class MaquinasDAO {
    ...
    async insert(maquina) {
        const sql = `INSERT INTO maquinas (fabricante, nome, agrupamento)
                     VALUES (?, ?, ?)`;
        const parametros = [maquina.fabricante ?? null, maquina.nome, maquina.agrupamento];
    ...
    async update(maquina) {
        const sql = `UPDATE maquinas
                     ...
                     WHERE id = ?`;
        const parametros = [maquina.fabricante ?? null, maquina.nome, maquina.agrupamento, maquina.id];
